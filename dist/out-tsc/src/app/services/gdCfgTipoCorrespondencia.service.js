...
var GdCfgTipoCorrespondenciaService = (function () {
    function GdCfgTipoCorrespondenciaService(_http, _loogerService) {
        this._http = _http;
        this._loogerService = _loogerService;
        this.url = environment_1.environment.apiUrl + 'gestiondocumental/gdcfgtipocorrespondencia';
    }
    GdCfgTipoCorrespondenciaService.prototype.index = function () {
        return this._http.get(this.url + "/").map(function (res) { return res.json(); });
    };
    GdCfgTipoCorrespondenciaService.prototype.register = function (datos, token) {
    ...
    GdCfgTipoCorrespondenciaService.prototype.delete = function (datos, token) {
        var json = JSON.stringify(datos);
        var params = "data=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
        return this._http.post(this.url + "/delete", params, { headers: headers }).map(function (res) { return res.json(); });
    };
    GdCfgTipoCorrespondenciaService.prototype.show = function (datos, token) {
        var json = JSON.stringify(datos);
        var params = "data=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
        return this._http.post(this.url + "/show", params, { headers: headers }).map(function (res) { return res.json(); });
    };
    GdCfgTipoCorrespondenciaService.prototype.edit = function (datos, token) {
        var json = JSON.stringify(datos);
        var params = "data=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
    ...
    GdCfgTipoCorrespondenciaService.prototype.select = function () {
        return this._http.get(this.url + "/select").map(function (res) { return res.json(); });
    };
    return GdCfgTipoCorrespondenciaService;
}());
GdCfgTipoCorrespondenciaService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http,
        logger_service_1.LoggerService])
], GdCfgTipoCorrespondenciaService);
exports.GdCfgTipoCorrespondenciaService = GdCfgTipoCorrespondenciaService;
//# sourceMappingURL=gdCfgTipoCorrespondencia.service.js.map