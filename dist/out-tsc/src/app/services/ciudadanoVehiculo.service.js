...
var CiudadanoVehiculoService = (function () {
    function CiudadanoVehiculoService(_http) {
        this._http = _http;
        this.url = environment_1.environment.apiUrl + "propietariovehiculo";
    }
    ...
    CiudadanoVehiculoService.prototype.register = function (token, datosT, tipoTraspaso) {
        var json = JSON.stringify(datosT);
    ...
    CiudadanoVehiculoService.prototype.showPropietarioByIdVehiculo = function (token, id) {
        var params = "authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
        return this._http.post(this.url + "/show/propietario/vehiculo/" + id, params, { headers: headers })
            .map(function (res) { return res.json(); });
    };
    CiudadanoVehiculoService.prototype.editLicenciaTransito = function (datos, token) {
    ...
    CiudadanoVehiculoService.prototype.eliminarVehiculoPropietario = function (token, datos) {
        var json = JSON.stringify(datos);
        var params = "json=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
