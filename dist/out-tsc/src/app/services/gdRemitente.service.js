...
var GdRemitenteService = (function () {
    function GdRemitenteService(_http, _loogerService) {
        this._http = _http;
        this._loogerService = _loogerService;
        this.url = environment_1.environment.apiUrl + "gestiondocumental/gdremitente";
    }
    GdRemitenteService.prototype.index = function () {
        return this._http.get(this.url + "/").map(function (res) { return res.json(); });
    };
    GdRemitenteService.prototype.register = function (datos, token) {
    ...
    GdRemitenteService.prototype.delete = function (datos, token) {
        var json = JSON.stringify(datos);
        var params = "data=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
    ...
    GdRemitenteService.prototype.show = function (datos, token) {
        var params = "authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
        return this._http.post(this.url + "/show", params, { headers: headers }).map(function (res) { return res.json(); });
    };
    GdRemitenteService.prototype.edit = function (datos, token) {
        var json = JSON.stringify(datos);
        var params = "data=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
    ...
    GdRemitenteService.prototype.searchByIdentificacion = function (datos, token) {
        var json = JSON.stringify(datos);
        var params = "data=" + json + "&authorization=" + token;
        var headers = new http_1.Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
        return this._http.post(this.url + "/search/identificacion", params, { headers: headers }).map(function (res) { return res.json(); });
    };
    return GdRemitenteService;
}());
GdRemitenteService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http,
        logger_service_1.LoggerService])
], GdRemitenteService);
exports.GdRemitenteService = GdRemitenteService;
//# sourceMappingURL=gdRemitente.service.js.map