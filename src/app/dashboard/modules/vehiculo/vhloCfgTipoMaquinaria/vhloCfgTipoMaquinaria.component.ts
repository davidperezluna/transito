...
export class VhloCfgTipoMaquinariaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._TipoMaquinariaService.index().subscribe(
				response => {
          this.tiposMaquinaria = response.data;
  ...
  onDelete(id:any){
    swal({
      ...
      if (result.value) {
        ...
        this._TipoMaquinariaService.delete({'id':id},token).subscribe(
            response => {
                swal({
                      title: 'Eliminado!',
