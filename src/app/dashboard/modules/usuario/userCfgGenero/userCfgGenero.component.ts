...
@Component({
  selector: 'app-index',
  templateUrl: './userCfgGenero.component.html'
})
export class UserCfgGeneroComponent implements OnInit {
  public errorMessage;
	public id;
	public respuesta;
	public generos;
  ...
  ngOnInit() {
    ...
    this._GeneroService.index().subscribe(
				response => {
  ...
  onDelete(id:any){
    swal({
      ...
      if (result.value) {
        ...
        this._GeneroService.delete({ 'id':id }, token).subscribe(
            response => {
                swal({
                      title: 'Eliminado!',
