...
@Component({
  selector: 'app-index',
  templateUrl: './cfgClave.component.html'
})
export class CfgClaveComponent implements OnInit {
  public errorMessage;
	public id;
	public respuesta;
	public claves;
  ...
  ngOnInit() {
    ...
    this._CfgClaveService.index().subscribe(
				response => {
  ...
  deleteInfraccionCategoria(id:any){
    swal({
      ...
      if (result.value) {
        ...
        this._CfgClaveService.delete(token,id).subscribe(
            response => {
                swal({
                      title: 'Eliminado!',
