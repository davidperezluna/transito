...
export class CfgValorVehiculoComponent implements OnInit {
  public errorMessage;
  public id;
  public respuesta;
  ...
  ngOnInit() {
    ...
    this._CfgValorVehiculoService.getCfgValorVehiculo().subscribe(
      response => {
        if (response) {
  ...
  deleteCfgValorVehiculo(id: any) {
    ...
    swal({
      ...
      if (result.value) {
        ...
        this._CfgValorVehiculoService.deleteCfgValorVehiculo(token, id).subscribe(
          response => {
            swal({
              title: 'Eliminado!',
