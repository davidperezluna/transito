...
export class VhloCfgTipoCabinaComponent implements OnInit {
  public errorMessage;
	public id;
  ...
  ngOnInit() {
    ...
    this._TipoCabinaService.index().subscribe(
				response => {
          this.tiposCabina = response.data;
  ...
  onDelete(id:any){
    swal({
      ...
      if (result.value) {
        ...
        this._TipoCabinaService.delete({'id':id},token).subscribe(
            response => {
                swal({
                      title: 'Eliminado!',
