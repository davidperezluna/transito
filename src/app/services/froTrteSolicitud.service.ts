...
export class FroTrteSolicitudService {
	...
	pdfExpedicionTarjetaControl(datos, token): any {
		let json = JSON.stringify(datos);
		let params = "data=" + json + "&authorization=" + token;
		...
		return this._http.post(this.url + "/pdf/expedicion/tarjeta/control", params, { 'responseType': ResponseContentType.Blob, headers: headers }).map(res => { return new Blob([res.blob()], { type: 'application/pdf' }) }
		);
