...
@Injectable()
export class UserEmpresaSucursalService {
	private url = environment.apiUrl + "usuario/userempresasucursal";
	public identity;
	public token;
	...
	getSucursalEmpresa(datos, token) {
		let json = JSON.stringify(datos);
		let params = "data=" + json + "&authorization=" + token;
		let headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
		return this._http.post(this.url + "/sucursales/by/empresa", params, { headers: headers }).map(res => res.json());
	}	
}