...
@Injectable()
export class FroTrteCfgConceptoService {
    private url = environment.apiUrl + 'financiero/frotrtecfgconcepto';
    public identity;
    public token;
    ...
    searchByTramitePrecio(datos, token) {
		let json = JSON.stringify(datos);
		let params = "data=" + json + "&authorization=" + token;
		let headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
		return this._http.post(this.url + "/search/tramiteprecio", params, { headers: headers }).map(res => res.json()
		);
	}
}
