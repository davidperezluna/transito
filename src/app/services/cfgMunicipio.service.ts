...
@Injectable()
export class CfgMunicipioService {
	private url = environment.apiUrl + "configuracion/cfgmunicipio";
	public identity;
	public token;
	...
	selectByDepartamento(datos, token) {
		let json = JSON.stringify(datos);
		let params = "data=" + json + "&authorization=" + token;
		let headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
		return this._http.post(this.url + "/select/departamento", params, { headers: headers }).map(res => res.json());
	}	
}