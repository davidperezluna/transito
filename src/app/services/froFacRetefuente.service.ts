...
@Injectable()
export class FroFacRetefuenteService {
	private url = environment.apiUrl + "financiero/frofacretefuente";
	public identity;
	public token;
	...
	searchByFactura(datos, token) {
		let json = JSON.stringify(datos);
		let params = 'data=' + json + '&authorization=' + token;
		let headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
		return this._http.post(this.url + '/search/factura', params, { headers: headers }).map(res => res.json());
	}
}